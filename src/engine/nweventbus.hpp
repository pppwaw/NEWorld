...
public:
    ...
    */
    ...
    void subscribe(const std::string& funcName, T func) {
        getSubscribers<T>(funcName).emplace_back(func);
    }
    ...
    */
    ...
    auto call(const std::string& funcName, Args&&... args) {
        auto list = getSubscribers<T>(funcName);
        Assert(list.size() == 1);
        return static_cast<T*>(list[0])(std::forward<Args>(args)...);
    }
    ...
    */
    ...
    void publish(const std::string& funcName, Args&&... args) {
        ...
        for(auto& subscriber: subscribers)
            static_cast<T*>(subscriber)(std::forward<Args>(args)...);
    }

private:
    template<typename T>
    auto getSubscribers(const std::string& funcName) noexcept {
        return mSubscribers[std::to_string(typeid(T).hash_code()) + "!" + funcName];
    }

    std::unordered_map<std::string, std::vector<std::add_pointer_t<void()>>> mSubscribers;
};
